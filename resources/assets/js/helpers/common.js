                    ...
                    $(form).find(":input[name='"+evaluacion[0]+"']").each(function (i, el) {
                        ...
                        for(var j in input){
                            if(evaluacion[3]=='string'){
                                if(input[j].value.includes(']')){
                                    if (input[j].value.includes(evaluacion[2])){
                                        resultado = true;
                                    }
                                }else{
            ...
            $(form).find(":input[name='" + campo + "']").each(function (i, el){
                ...
                for (var j in input) {
                    if(tipo == "regex"){
                        ...
                        if(typeof(campo)=== 'string' && campo.includes('[]') && input[j].value != ""){
                            if(input[j].value.includes(','))
                            {
                                var obj = JSON.parse(input[j].value);
                                if (relacion == "==" && input[j].value && obj.includes(valor)){
                                    visible = true;
                                }
                            }else{
...
var procesar_data = function(data){
    ...
    $.each(data, function(k,v) {
        if( data[k].valor && $('[name="'+data[k].nombre+'"]').length){
            ...
            // parche para radio button + comportamiento de btn async
            else if($('[name="'+data[k].nombre+'"]').length > 1 && $('[name="'+data[k].nombre+'"]')[0].type=="radio") {
                $('[name="'+data[k].nombre+'"]').each(function(index) {
                    if ('"'+$(this).val()+'"' == data[k].valor) {
