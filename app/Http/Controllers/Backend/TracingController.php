...
class TracingController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if(in_array("seguimiento",$roles) && count($procesos) == 0 && !$process_search){
        ...
        return view('backend.tracing.index', [
            'procesos' => $procesos,
    ...
    public function indexProcess(Request $request, $proceso_id) {
        ...
        $roles = explode(",",Auth::user()->rol);
        if(in_array("modelamiento",$roles) || in_array("seguimiento",$roles))
        {
            ...
            {
                echo 'Usuario no tiene permisos para hacer seguimiento a este proceso';
                exit;
            }
        }
