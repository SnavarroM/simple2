...
class AuditController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $registros = AuditoriaOperaciones::where('cuenta_id', Auth::user()->cuenta_id);

        if ($search) {
            $registros = $registros->where('proceso', 'like', "%$search%")
        ...
        if ($order && $direction)
            $registros = $registros->orderBy($order, $direction);
        else
