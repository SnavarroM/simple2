...
class ConfigurationController extends Controller
{
    ...
    public function addBackendUsers()
    {
        ...
        $data['procesos'] = Proceso::where('cuenta_id',Auth::user()->cuenta_id)->where('activo',1)->get();
        $data['edit'] = false;
    ...
    {
        $data['form'] = UsuarioBackend::where('id',$id)->first();
        $data['procesos_selected'] = $data['form']->procesos_permiso()->get()->groupBy('id')->keys()->toArray();
        $data['procesos'] = Proceso::where('cuenta_id',Auth::user()->cuenta_id)->where('activo',1)->get();
        $data['edit'] = true;
    ...
    public function saveBackendUsers(Request $request, UsuarioBackend $user, $edit = false)
    {
        ...
        if (!$edit) {
            ...
            $user->last_login = \Carbon\Carbon::now('America/Santiago')->format('Y-m-d H:i:s');
        }
        ...
        $user->is_disabled = $request->has('is_disabled') ? true : false;
        $user->cuenta_id = Auth::user()->cuenta_id;
        ...
        if(in_array("modelamiento",$roles) || in_array("seguimiento",$roles) || in_array("gestion",$roles))
        {
            if ($request->has('procesos')) {
                foreach ($request->input('procesos') as $id) {
                    $user->procesos_permiso()->attach($id);
                }
            }
        }
