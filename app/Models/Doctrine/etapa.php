...
class Etapa extends Doctrine_Record
{
    ...
    function setTableDefinition()
    {
        ...
        $this->hasColumn('vencimiento_avance');
    }
            ...
            if ($tp->estado == 'completado') {
                ...
                if ($tp->estado == 'pendiente') {
                    ...
                    foreach ($tareas_proximas as $tarea_proxima) {
                        ...
                        if(!is_null($etapa->vencimiento_at))
                            $etapa->vencimiento_avance = 'pendiente';
                        $etapa->save();
                        ...
                        if ($usuario_asignado_id){
                            ...
                            $subject = 'Tiene una tarea pendiente de asignación';
                            $mensaje_pendiente = 'tiene una tarea pendiente por realizar llamada';
                            $etapa->notificarTareaPendiente($subject,$mensaje_pendiente);
                        }
    ...
    public function asignar($usuario_id)
    {
        ...
        if ($this->Tarea->asignacion_notificar) {
            ...
            if ($usuario->email) {
               ...
               $mensaje = 'se te ha asignado la tarea llamada';
               $cuenta = $this->Tramite->Proceso->Cuenta;
               $nombre_cuenta = Cuenta::cuentaSegunDominio()->nombre_largo;
               $this->notificarTareaPendiente($subject,$mensaje);
    ...
    public function notificarTareaPendienteOld()
    {
        if ($this->Tarea->asignacion_notificar) {
    ...
    public function notificarTareaPendiente($asunto,$mensaje)
    {
        if ($this->Tarea->asignacion_notificar) {
            ...
            foreach ($usuarios as $usuario) {
                \Log::info('inicio notificación tarea pendiente');
                if ($usuario->email) {
                    $varurl = '';
                    if ($this->usuario_id) {
                        $varurl = url('etapas/ejecutar/' . $this->id);
                    ...
                    $message = 
                            ...
                            <div style="text-indent:40px;line-height:0px"><img src="'.$logo.'"/></div>
                                ...
                                <br>
                                <div style="text-indent:40px;text-align:justify;font-family: Roboto, sans-serif;font-size: 16px;line-height: 24px;">Hola, '.$mensaje_pendiente.' </div>
                                ...
                                <div style="text-indent:40px;text-align:justify;font-family: Roboto, sans-serif;font-size: 16px;line-height: 24px;"> <b>'.$cuenta->nombre.'</b></div>
                                </p>
                                </div>
                            </div>';
                    ...
                    // }
                    // unset($data_body);
                    \Log::info('fin notificación tarea pendiente');
                }
            }
        }
    }
