...
class IntegracionMediator
{
    ...
    public static function registerUserFromHeadersClaveUnica(stdClass $body)
    {
        ...
        if ($user == NULL) {  //Registrar el usuario
            ...
            $user->rut = $body->rut;
            $apellido_trim = str_replace("; ", " ", $body->apellidos);
            $apellidos = explode(";", $apellido_trim);
            ...
            try{
                if (count($apellidos) < 2) {
                    ...
                    $returnData = array(
                        'status' => '400',
                        'message' => 'Las credenciales de ClaveUnica estan incompletas'
                    );
                   throw new ApiException(Response::json($returnData, 400));
                }
            } catch (Exception $e){
               echo 'Mensaje de error: ' . $e->getMessage() . "<br>";
            }
            Log::info('Apellido0 $apellido_materno->apellido_materno: ' . $user->apellido_materno);
            Log::info('Apellido0 $user->apellido_paterno: ' . $user->apellido_paterno);
