...
class CampoFileS3 extends Campo
{
    ...
    protected function display($modo, $dato, $etapa_id = false)
    {
        ...
        if($dato && isset($dato->valor) && isset($dato->valor->URL) && strlen($dato->valor->URL) > 1){
            ...
            $saved_filename = isset($dato->valor->filename) ? $dato->valor->filename: $saved_filename; // string
        }else if ($etapa_id ){
            ...
            $saved_info = json_encode($saved_info_obj);
            $saved_filename = isset($data->filename) ? $data->filename: $saved_filename; // string
        }
        ...
        $display .= '
            ...
            if( ! is_null($saved_url)){
                $display .= 'set_s3_hidden('.$this->id.',"'.$saved_url.'",'.$saved_info.',"'.$saved_filename.'");';
            }
            $display .= '
                });</script>';
