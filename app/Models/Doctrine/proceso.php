...
class Proceso extends Doctrine_Record
{
    function setTableDefinition()
    {
        ...
        $this->hasColumn('nombre_frontend');
        $this->hasColumn('codigo_rnt');
        ...
        $this->hasColumn('descarga_documentos');
        ...
        if(\Schema::hasColumn('proceso', 'descarga_documentos')){
            $this->hasColumn('descarga_documentos');
        }
    }
    ...
    public static function importComplete($input, $isImport = FALSE)
    {
        ...
        foreach ($json as $keyp => $p_attr) {
            if ($keyp == 'Tareas') {
                ...
                if ($keyp == 'nombre') {
                    $fecha = new Datetime();
                    $proceso->{$keyp} = $p_attr . ' ' . $fecha->format("d-m-Y H:i:s") . ''; // issue https://git.gob.cl/simple/simple/issues/622
                } elseif ($keyp == 'root') {
                    $proceso->{$keyp} = null;
                } elseif ($keyp == 'version') {
                    $proceso->{$keyp} = 1;
