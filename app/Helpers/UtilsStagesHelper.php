...
function isBandejaCategoryActive($type='')
{
    $bandejaActive = Request::session()->get('bandeja_active', null);

    return ($bandejaActive == $type) ? 'active':'';
}

function isBandejaActive() {
    return Request::session()->exists('bandeja_active') ? true:false;
}

function getBandejaActiveName()
{
    $breadcrumbCategory = '';

    $bandejaActive = Request::session()->get('bandeja_active', null);

    if (null != $bandejaActive) {
        switch ($bandejaActive) {
            case 'home':
                $breadcrumbCategory = 'Trámites Disponibles';
                break;
            case 'inbox':
                $breadcrumbCategory = 'Bandeja de Entrada';
                break;
            case 'sin_asignar':
                $breadcrumbCategory = 'Sin Asignar';
                break;
            case 'historial':
                $breadcrumbCategory = 'Historial de Trámites';
                break;
        }
    }
