...
class FilesDownload implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        foreach ($this->tramites as $tramite){
            ...
            if($archivos_tramite[0]['nice_directory']=='subidos_file_transfer'){
                $source = self::download_s3_files_to_zip_folder($dir_tramite_files, $archivos_tramite, $this->added_files_path);
            }else{
    ...
    private function send_notification(){
      ...
      $data = ['content' => $this->email_content, 'link' => $link ,'reportname' => $this->cuenta->nombre_largo];
      $cuenta = $this->cuenta;
      Mail::send('emails.download_files', $data, function($message) use ($cuenta,$link){
    ...
    public static function download_s3_files_to_zip_folder($out_dir, $file_list, &$copied_files){
        \Log::info('Inicio descarga desde S3');
        ...
        foreach($file_list as $f_array[0]){
            foreach ($f_array as $file) {
                ...
                $dest_file = $dir.DIRECTORY_SEPARATOR.$file['nice_name'];
        ...
        \Log::info('Fin descarga desde S3');
        return $out_dir;
    }
